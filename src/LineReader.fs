...
let private common startIndex (candidates: string list) =
    ...
    let uncommonPoint =
        ...
        |> List.find (fun i ->
            if i >= candidates.[0].Length then
                true
            else
                let charAt = candidates.[0].[i]

                List.tryFind (fun (c: string) -> c.Length = i || c.[i] <> charAt) candidates
                <> None
        )
...
let private attemptTabCompletion soFar pos =
    ...
    let asPath =
        if Path.IsPathRooted last then
            last
        else
            Path.Combine(currentDir (), last)
    ...
    try
        let files =
            Directory.GetFiles asDirectory |> Array.map Path.GetFileName |> Array.toList

        let folders =
            Directory.GetDirectories asDirectory
        ...
        let candidates =
            allOptions |> List.filter (fun n -> n.ToLower().StartsWith(asFile.ToLower()))
        ...
        if candidates.Length = 0 then
            ...
            let matched =
                if candidates.Length = 1 then
                    ...
                    common asFile.Length candidates
            ...
            soFar, soFar.Length
    with :? IOException ->
        soFar, pos // Invalid path or file access, so treat as not completable.
...
let readLine (prior: string list) =
    ...
    let linesToClear =
        "" :: prior |> Seq.map (fun p -> p.Split newline |> Seq.length) |> Seq.max
    ...
    let lastLineStart (soFar: string) =
        ...
        if lastLineBreak = -1 then
            0
        else
            lastLineBreak + newline.Length
    ...
    let rec reader priorIndex (soFar: string) pos =
        ...
        | ConsoleKey.Enter -> reader priorIndex (soFar + newline) 0
        ...
        | ConsoleKey.Delete ->
            ...
            let nextSoFar =
                if soFar = "" || relPos = soFar.Length then
                    soFar
                else
                    soFar.[0 .. relPos - 1] + soFar.[relPos + 1 ..]
        ...
        | ConsoleKey.Home -> reader priorIndex soFar 0
        | ConsoleKey.End ->
        ...
        | ConsoleKey.Tab when soFar <> "" ->
            let newSoFar, newPos =
                ...
                | Some (Code code) ->
                    if not (code.Contains newline) then
                        soFar, pos
                    else
                        let lineStart = lastLineStart soFar

                        let newSoFar =
                            soFar.[.. lineStart - 1] + String(' ', codeSpaces) + soFar.[lineStart..]

                        newSoFar, pos + codeSpaces
                | _ -> attemptTabCompletion soFar pos
