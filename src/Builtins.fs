...
let private clear _ _ _ = Console.Clear()
...
let private dir args writeOut _ =
    let searchPath =
        Path.Combine(currentDir (), (if List.isEmpty args then "" else args.[0]))
    ...
    if File.Exists searchPath then
        ...
        let finalPath, finalPattern =
            ...
            else if searchPattern = "*" then
                Path.GetDirectoryName searchPath, Path.GetFileName searchPath
            else
                Path.GetDirectoryName searchPath, searchPattern
...
let private cd args _ writeError =
    if List.isEmpty args then
        writeError "no path specified"
    elif args.[0] = "\\" && isWindows then
...
let builtins = [
    ...
    "rm",
    (rm, "same as del, deletes the target file or directory (use rm -f [dir] to delete a directory containing files)")
    "del",
    (rm, "same as rm, deletes the target file or directory (use del -f [dir] to delete a directory containing files)")
    ...
    [
        if List.isEmpty args then
            ...
            yield!
                ...
                |> List.map (fun result -> result ||> sprintf "%s: %s")
    ]
    |> Seq.iter writeOut
