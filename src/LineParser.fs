...
let parts s =
    ...
    let rec parts soFar wrapped last remainder acc =
        if remainder = "" then
            ...
            | '\"', None when soFar = "" -> parts soFar (Some('\"', 1)) last next acc // Quotes always have a 'stack' of 1, as they cant be pushed/popped like brackets.
            | '\"', Some ('\"', 1) when last <> '\\' ->
                parts "" None last next (fun next -> acc (sprintf "\"%s\"" soFar :: next))
            // if in code (wrapping '(') then quotes indicate a string. { and [ prevent the bracket push failing
            | '\"', Some ('(', n) -> parts (soFar + "\"") (Some('{', n)) last next acc
            | '\"', Some ('{', n) -> parts (soFar + "\"") (Some('(', n)) last next acc
            | ''', Some ('(', n) -> parts (soFar + "'") (Some('[', n)) last next acc
            | ''', Some ('[', n) -> parts (soFar + "'") (Some('(', n)) last next acc
            // spaces sperate parts, as long as not in a wrapped section
            | ' ', None when last <> '\\' -> parts "" None last next (fun next -> acc (soFar :: next))
            | '\n', None when soFar = "\r" || soFar = "" -> parts "" None last next (fun next -> acc (newline :: next))
            | _ -> parts (soFar + string c) wrapped c next acc
    ...
    if List.isEmpty raw then raw else joinBlanks raw // A final fold is used to combine whitespace blocks: e.g. "";"";"" becomes "   "
...
let tokens partlist =
    let rec tokens partlist acc =
        ...
        | head :: remainder ->
            match head with
            | (s: string) when s.StartsWith newline -> tokens remainder (fun next -> acc (Linebreak :: next))
            | s when String.IsNullOrWhiteSpace s -> tokens remainder (fun next -> acc (Whitespace s.Length :: next))
            ...
            | command ->
                let rec findArgs list =
                    ...
                    | []
                      ...
                      | ">" :: _
                      | ">>" :: _ -> []
                    | "" :: remainder -> " " :: findArgs remainder
                    | head :: remainder -> head :: findArgs remainder
