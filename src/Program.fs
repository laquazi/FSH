...
let main _ =
    ...
    printf "version: "
    printc Colours.goodOutput ("2019.5.22.2" + newline)

    // Booting FSI takes a short but noticeable amount of time.
    printf "starting "
    printc Colours.code "FSI"
    printf "..."
    let fsi = Fsi()
    printfn "done"
    ...
    let runCommand command args lastResult writeOut writeError =
        ...
        if command = "help" || command = "?" then
            ...
            | Some f ->
                ...
                with :? UnauthorizedAccessException as ex ->
    ...
    let processToken isLastToken lastResult token =
        ...
        | Error ex ->
            if isLastToken then
                writeError ex // no processing occurs after the last result, so the error needs to be printed
        ...
        | Ok s ->
            ...
            | Code code -> runCode s code writeOut writeError
            | Out (append, path) -> runOut s append path writeOut writeError
            | Pipe
            | Whitespace _
            | Linebreak ->
    ...
    let executeEntered (s: string) =
        if String.IsNullOrWhiteSpace s then
            () // nothing specified so just loop
        else
            let parts = parts s
            ...
            ||> List.fold (fun lastResult token -> processToken (token = lastToken) lastResult token)
